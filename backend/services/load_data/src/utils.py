...
async def search_buildigs(
    ...
    try:
        if building_filter.d__in:
            building_filter.d__in = ",".join(
                [
                    "д." + house_number
                    for house_number in building_filter.d__in.split(",")
                ]
            )
        if building_filter.k__in:
            building_filter.k__in = ",".join(
                [
                    "к." + house_number
                    for house_number in building_filter.k__in.split(",")
                ]
            )
        ...
        if buildings_with_rec:
            buildings_parsed = [
                {
                    "build": building.get("build").to_dict(
                        ignored_fields=["id", "d", "unom", "k", "s", "col_754"]
                    ),
                    "reccomendation": building.get("recommendation").to_dict(
                        ignored_fields=["id"]
                    ),
                }
                for building in buildings_with_rec
            ]
            new = []
...
async def get_excel_file(file_id: str):
    ...
    for i in range(0, len(header_list)):
        worksheet.set_column(
            i, i, len(header_list[i]) + 10
        )  # Set column widths based on len(header)
