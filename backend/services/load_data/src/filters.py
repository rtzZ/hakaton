...
class BuildingFilter(Filter):
    ...
    address__like: Optional[str] = Field(
    ...
    d__in: Optional[list[str]] = Field(
        Query(None, description="Номер дома"), alias="house_number"
    )
    k__in: Optional[list[str]] = Field(
        Query(None, description="Номер корпуса"), alias="corpus_number"
    )

    col_771__lt: Optional[int] = Field(
        Query(None, description="Количество лифтов"), alias="lift_count_no"
    )  # 1 No
    col_771__gt: Optional[int] = Field(
        Query(None, description="Количество лифтов"), alias="lift_count_yes"
    )  # 0 Yes
    ...
    col_770__ilike: Optional[str] = Field(
        Query(None, description="Аварийность"), alias="accident_id"
    )
    unom__in: Optional[list[int]] = Field(
